...
class Database{
  ...
  /**
   * Método responsável por executar uma consulta com inner join no banco
   * @param  string $fields
   * @param  string $inner
   * @return PDOStatement
   */
  public function inner($inner = null, $where = null, $fields = '*'){
    //DADOS DA QUERY
    $inner = strlen($inner) ? 'INNER JOIN '.$inner : '';
    $where = strlen($where) ? 'WHERE '.$where : '';

    //MONTA A QUERY
    $query = 'SELECT '.$fields.' FROM '.$this->table.' '.$inner.' '.$where;
